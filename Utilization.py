...
class onlineTimeline: 
    ...
    def loadGraph(self):
        ...
        with open(self.inputfile,"r") as f:
            ...
            for i in range(0,len(taskGraph1)):
                ...
                self.totalReceive = self.totalSize + taskGraph1[str(i)]['total_needReceive']
                ...
                # print(i,taskGraph1[str(i)]['out_links'])

                

                if(len(taskGraph1[str(i)]['out_links'])!=0):
                    for task in taskGraph1[str(i)]['out_links']:
                        # print(taskGraph1[str(i)]['out_links'][0],"task++++",task,i)
                        # print(task)
                        if(len(task[0][3])!=0):
                            linkName = str(i)+"to"+str(task[0][0]) + task[0][3][0][1]
                        else:
    ...
    def sending(self,sendinfos):
        ...
        for item in sendinfos:
            #linkName = str(item[0])+"to" + str(item[1]) + item[0][3][0][1]
            #print(item)
            if(len(item[2])!=0):
                linkName = str(item[0])+"to" + str(item[1]) + item[2][0][1]
            else:
    ...
    #finished 3  
        ...
        for i in sendThisTime:
            ...
            for dest in self.taskGraph[str(i)]['out_links']:
                ...
                if(len(dest[0][3])!=0):
                    linkName = str(i)+"to"+str(dest[0][0]) + dest[0][3][0][1]
                else:
                    linkName = str(i)+"to"+str(dest[0][0])
                
                # print("linkName++++++++: ",i, linkName)
                # print(dest)
                # print(self.taskGraph[str(i)]['out_links'])
                # print("++++++++++++++++++++++++++")
        ...
        for i in self.pendTask[:]:
            ...
            if(canSend==1):
                ...
                if(len(i[2])!=0):
                    linkName = str(i[0])+"to"+str(i[1]) + i[2][0][1]
                else:
    ...
    def computeTime(self):
        ...
        while(remain>0 and z<10000):#
            ...
            utilize.append([self.nowTime,self.getUtilization()])

            self.nowTime=self.nowTime+1
        if(z==10000):
            print("outlinke: ",self.outLinks)
            print("exeMatric: ",exeThisTime,self.exeMatric)
            print("sendMatric: ",sendThisTime)
            print("sendMatrix: ",self.sendMatrix)
            print("receiveMatrix: ",self.receiveMatrix)
            print("sendList: ",self.outLinks)
         
            print("not stop normally!")
        print("Finish!!! cycles: ",z)
        print("totalSend: ",self.totalSize)
        print("totalReceive: ",self.totalReceive)
        with open("result1.txt","a") as t:
            t.write(self.inputfile)
            t.write("\n Utilization Total time: "+str(self.nowTime)+"\n")
...
def main(argv):
    ...
    inputfile = 'test_1_split_result/autocor_Mesh8x8_AIR1_free.json'
    rowNum = '8'
    outputfile = 'test_1_split_result_utilize_result/audiobeam_Mesh4x4_AIR3_free.json'
